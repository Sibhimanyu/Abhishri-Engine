...
window.studentDirectory = {
        ...
        if (subtitle) {
            if (this.currentView === 'report' && this.currentStudentId) {
                ...
                const labels = {
                    ...
                    manage: 'Manage admissions and records',
    ...
    renderDirectory() {
        ...
        if (sortedIds.length === 0) {
            container.innerHTML = `<div class="empty-state"><i data-lucide="users"></i><p>${this.searchQuery ? 'No students match your search.' : 'No students enrolled yet. Go to Manage Students to add records.'}</p></div>`;
            return;
        }
    ...
    renderManage() {
        ...
        const toolbar = document.getElementById('student-toolbar');
        if (toolbar) {
            toolbar.innerHTML = `<div class="search-box"><i data-lucide="search"></i><input type="text" placeholder="Search admissions..." oninput="window.studentDirectory.handleSearch(this.value)" value="${this.searchQuery}"></div>
                <div style="display:flex; gap:10px;"><button class="btn btn-secondary" onclick="window.studentDirectory.seedSampleStudent()"><i data-lucide="database"></i> Seed</button><button class="btn btn-primary" onclick="window.studentDirectory.showStudentForm()"><i data-lucide="user-plus"></i> New Admission</button></div>`;
        }
    ...
    async renderReport(id) {
        ...
        let html = `
            <div class="profile-container" style="padding-bottom: 60px;">
                ...
                <!-- Financial & Performance (Interlinked) -->
                <div class="profile-info-grid" style="display:grid; grid-template-columns: 1.5fr 1fr; gap:32px; margin-top: 32px;">
                    ...
                    <div class="profile-info-card" style="background:var(--surface); border:1px solid var(--card-border); padding:32px; border-radius:24px;">
                        ...
                        ${latestPulses.length > 0 ? latestPulses.map(p => `
                            <div style="padding:12px; background:rgba(255,255,255,0.02); border-radius:12px; margin-bottom:10px; border-left:3px solid var(--accent-secondary);">
                                <div style="display:flex; justify-content:space-between; font-size:0.75rem; margin-bottom:4px;"><span style="font-weight:700;">${p.category}</span><span style="opacity:0.6;">${p.date}</span></div>
                                <div style="font-size:0.85rem; line-height:1.4; color:var(--text-main);">${p.notes}</div>
                            </div>
                        `).join('') : '<div class="empty-state" style="padding:20px;"><p>No development logs found.</p></div>'}
    ...
    renderPerformance() {
        ...
        if (this.currentStudentId) {
            this.renderStudentPulse(this.currentStudentId);
            return;
        }
    ...
    async renderGlobalPulse() {
        ...
        snap.forEach(doc => {
            ...
            html += `<div class="console-card" style="padding:20px; border-left: 4px solid var(--accent-secondary);">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <div><strong style="font-size:1.1rem; color:var(--text-main);">${s.name}</strong> <span style="font-size:0.8rem; color:var(--text-dim); margin-left:10px;">• ${p.category}</span></div>
                    <div style="font-size:0.8rem; opacity:0.6;">${p.date}</div>
                </div>
                <p style="margin:0; line-height:1.5;">${p.notes}</p>
                <div style="margin-top:12px; font-size:0.7rem; color:var(--text-dim); display:flex; justify-content:space-between;"><span>Log ID: ${doc.id.slice(-6).toUpperCase()}</span><span>Recorded by: ${p.createdBy}</span></div>
            </div>`;
        });
    ...
    async renderStudentPulse(id) {
        ...
        const canLog = isAdmin || perfPerms === true || perfPerms.log;

        const toolbar = document.getElementById('student-toolbar');
        if (toolbar) {
            toolbar.innerHTML = `<button class="btn btn-secondary" onclick="window.studentDirectory.switchView('report', '${id}')"><i data-lucide="arrow-left"></i> Profile</button>
    ...
    showPulseEntryForm(id) {
        const s = this.students[id];
        const content = `
            <div class="form-group"><label>Date</label><input type="date" id="p-date" class="form-control" value="${new Date().toISOString().split('T')[0]}"></div>
            <div class="form-group" style="margin-top:15px;"><label>Focus Category</label><select id="p-cat" class="form-control"><option>Academics</option><option>Social & Behavioral</option><option>Physical Development</option><option>Creative Arts</option><option>Discipline</option></select></div>
            <div class="form-group" style="margin-top:15px;"><label>Rating (1-5)</label><input type="number" id="p-rating" class="form-control" value="3" min="1" max="5"></div>
            <div class="form-group" style="margin-top:15px;"><label>Observations & Notes</label><textarea id="p-notes" class="form-control" rows="4" placeholder="What happened today?"></textarea></div>`;
        
        AppDialog.confirm({
            title: `Log Development: ${s.name}`,
            content,
            onConfirm: async () => {
                const logData = {
                    studentId: id,
                ...
                AppDialog.toast('Pulse recorded', 'success');
                this.renderStudentPulse(id);
                return true;
            }
        });
    },
    ...
    showStudentForm(id = null) {
        ...
        const content = `
            <div class="form-scroll-container" style="max-height: 70vh; overflow-y: auto; padding-right: 15px;">
                ...
                <div style="background:rgba(255,255,255,0.02); padding:15px; border-radius:12px; margin-bottom:15px;">
                    ...
                    <div class="form-grid-2" style="margin-top:10px;"><input type="text" id="sf-m-phone" class="form-control" value="${s.motherPhone || ''}" placeholder="Phone Number"><input type="email" id="sf-m-email" class="form-control" value="${s.motherEmail || ''}" placeholder="Email Address"></div>
                </div>
                <div style="background:rgba(255,255,255,0.02); padding:15px; border-radius:12px;">
                    ...
                    <div class="form-grid-2" style="margin-top:10px;"><input type="text" id="sf-f-phone" class="form-control" value="${s.fatherPhone || ''}" placeholder="Phone Number"><input type="email" id="sf-f-email" class="form-control" value="${s.fatherEmail || ''}" placeholder="Email Address"></div>
                </div>

                <div class="form-section-title"><i data-lucide="phone-call"></i> 4. Emergency Contact</div>
                <div class="form-grid-2">
                    <div class="form-group"><label>Name</label><input type="text" id="sf-e-name" class="form-control" value="${s.emergencyContactName || ''}"></div>
                    <div class="form-group"><label>Relationship</label><input type="text" id="sf-e-rel" class="form-control" value="${s.emergencyRelationship || ''}"></div>
                </div>
                ...
                <div class="form-grid-2">
                    <div class="form-group"><label>Sibling 1 Name</label><input type="text" id="sf-s1-name" class="form-control" value="${s.sibling1Name || ''}"></div>
                    <div class="form-group"><label>Sibling 1 Age/School</label><input type="text" id="sf-s1-det" class="form-control" value="${s.sibling1Detail || ''}"></div>
                </div>
                ...
                <div class="form-grid-2">
                    <div class="form-group"><label>Physician Name</label><input type="text" id="sf-phys-name" class="form-control" value="${s.physicianName || ''}"></div>
                    <div class="form-group"><label>Physician Phone</label><input type="text" id="sf-phys-phone" class="form-control" value="${s.physicianPhone || ''}"></div>
                </div>

                <div class="form-section-title"><i data-lucide="shield-check"></i> 9. Authorized Pickups</div>
                <div class="form-grid-2">
                    <div class="form-group"><label>Pickup 1 Name</label><input type="text" id="sf-p1-name" class="form-control" value="${s.pickup1Name || ''}"></div>
                    <div class="form-group"><label>Relationship</label><input type="text" id="sf-p1-rel" class="form-control" value="${s.pickup1Rel || ''}"></div>
                </div>
        ...
        AppDialog.confirm({
            ...
            onConfirm: async () => {
                const data = { 
                    ...
                    allergiesList: document.getElementById('sf-allergies').value,
                    medicalConditions: document.getElementById('sf-medical').value,
                    ...
                    appNumber: document.getElementById('sf-app-no').value,
                    verifiedBy: document.getElementById('sf-verified').value,
                ...
                AppDialog.toast('Record saved', 'success');
                return true;
            }
        });
    },
    ...
    renderAttendanceReports() {
        ...
        if (toolbar) {
            toolbar.innerHTML = `
                ...
                <button class="btn btn-secondary" onclick="window.print()"><i data-lucide="printer"></i> Print Analysis</button>
            `;
        }
                ...
                <div class="report-body">
                    <div class="metrics-grid">
                        <div class="metric-card">
                            ...
                            <div class="metric-info"><h3>Total Students</h3><div class="metric-value">${Object.keys(this.students).length}</div></div>
                        </div>
                        <div class="metric-card">
        ...
        container.innerHTML = `
            <div class="report-page">
                <div class="report-hero">
                    ...
                    <p style="color:var(--text-dim); font-size:1.1rem;">Student presence overview for the last 7 days</p>
                </div>
    ...
    renderMonthlyReport(container) {
        container.innerHTML = '<div class="empty-state"><i data-lucide="bar-chart-3"></i><p>Monthly analytics view is being processed.</p></div>';
    },
    ...
    deleteStudent(id) {
        ...
        AppDialog.confirm({
            title: 'Delete Student', msg: `Permanently delete ${s.name}?`, danger: true, onConfirm: async () => {
                ...
                window.AppLogger.log('DELETE_STUDENT', 'student_directory', { name: s.name }, id);
                return true;
            }
        });
    },
