...
if (!window.whatsAppSender) {
    ...
    window.whatsAppSender.renderLists = function () {
        ...
        if (newLists.length > 0) {
            htmlStr += `<div style="font-size:0.9rem; font-weight:600; color:var(--text-dim); margin-bottom:12px; margin-top:4px;">Active Lists</div><div class="wa-lists-grid">${newLists.map((list, i) => {
        ...
        if (usedLists.length > 0) {
            htmlStr += `<div style="font-size:0.9rem; font-weight:600; color:#ef4444; margin-bottom:12px; margin-top:24px; display:flex; gap:6px; align-items:center;"><i data-lucide="lock" style="width:14px;height:14px;"></i> Locked / Used Lists</div><div style="font-size:0.8rem; color:var(--text-dim); margin-bottom:12px;">These lists have been used in past broadcasts and are permanently locked to preserve analytics.</div><div class="wa-lists-grid">${usedLists.map((list, i) => {
                const color = colors[i % colors.length];
                const initials = (list.name || '?').substring(0, 2).toUpperCase();
                const count = list.contactsCount || list.count || 0;
                const numbersCount = list.numbersCount || count;
                const dateStr = list.createdAt ? (list.createdAt.toDate ? list.createdAt.toDate() : new Date(list.createdAt)).toLocaleDateString() : 'N/A';
                return `<div class="wa-list-row" onclick="window.whatsAppSender.viewListDetails('${list.id}')" style="opacity: 0.7; background: rgba(0,0,0,0.15);"><div class="wa-list-avatar" style="background:${color}22;color:${color};">${initials}</div><div class="wa-list-info"><div class="wa-list-name">${list.name}</div><div class="wa-list-sub" style="display:flex; flex-direction:column; gap:2px;"><span>${count} contact${count !== 1 ? 's' : ''} • ${numbersCount} unique number${numbersCount !== 1 ? 's' : ''}</span><span style="font-size:0.7rem; opacity:0.6;">Created ${dateStr}</span></div></div><div class="wa-list-badge" style="background:rgba(255,255,255,0.05);color:var(--text-dim);">${numbersCount}</div><i data-lucide="lock" style="width:14px;height:14px;color:rgba(239,68,68,0.8);flex-shrink:0;"></i></div>`;
            }).join('')}</div>`;
        }
