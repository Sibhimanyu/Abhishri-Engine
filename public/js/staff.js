...
window.staffDirectory = {
    ...
    async render() {
        ...
        if (!isAdmin) {
            ...
            if (this.currentView === 'manage') {
                hasEntry = staffPerms.add || staffPerms.edit || staffPerms.delete;
            } else {
    ...
    renderAttendanceReports() {
        const container = document.getElementById('staff-content-attendance-reports'); if (!container) return;
        const toolbar = document.getElementById('staff-toolbar');
    ...
    renderPerformance() {
        const id = this.currentStaffId; if (id) { this.renderStaffPulse(id); return; }
        const container = document.getElementById('staff-content-performance'), toolbar = document.getElementById('staff-toolbar');
        ...
        let html = `<div class="report-header" style="margin-bottom:32px;"><h2 style="font-size:2rem; font-weight:800; margin-bottom:8px;">Staff Performance Pulse</h2><p style="color:var(--text-dim); font-size:1.1rem;">Track achievements, professional growth, and activity logs.</p></div><div class="directory-grid">`;
        sortedIds.forEach(id => {
            const s = this.staff[id];
            html += `<div class="directory-card" onclick="window.staffDirectory.switchView('performance', '${id}')" style="border-left:4px solid var(--accent-secondary); display:flex; flex-direction:column; justify-content:space-between; height:100%;"><div class="member-header"><div class="member-avatar" style="background:rgba(115, 199, 200, 0.15); color:var(--accent-secondary); font-size:1.5rem;">${(s.name || 'S')[0]}</div><div class="member-info"><h3>${s.name}</h3><p>${s.designation || 'No Role'}</p></div></div><div class="member-actions" style="margin-top:auto; padding-top:12px; border-top:1px solid var(--card-border);"><span style="color:var(--accent-secondary); font-size:0.85rem; font-weight:700; display:flex; align-items:center; gap:6px;">VIEW ACTIVITY <i data-lucide="arrow-right-circle" style="width:16px; height:16px;"></i></span></div></div>`;
        });
        container.innerHTML = html + '</div>';
    },

    async renderStaffPulse(id) {
        const container = document.getElementById('staff-content-performance'), s = this.staff[id]; if (!s) return;
        const userData = window.currentUserData || {}, isAdmin = userData.isAdmin, staffPerms = userData.permissions?.staff_directory || {}, isMaster = isAdmin || staffPerms === true;
        const toolbar = document.getElementById('staff-toolbar');
        if (toolbar) {
            const canPulse = isMaster || staffPerms.pulse; // Entry needs view, log needs pulse (see form)
            toolbar.innerHTML = `<button class="btn btn-secondary" onclick="window.staffDirectory.switchView('performance')"><i data-lucide="arrow-left"></i> All Staff</button>${(isAdmin || staffPerms.pulse) ? `<button class="btn btn-primary" onclick="window.staffDirectory.showPulseEntryForm('${id}')"><i data-lucide="plus-circle"></i> Log Performance</button>` : ''}`;
        }
        let logsHtml = '';
        try {
            ...
            snap.forEach(doc => { const d = doc.data(); logsHtml += `<div class="console-card" style="margin-bottom:20px; border-left:4px solid var(--accent-secondary); padding:20px;"><div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span style="font-size:0.75rem; font-weight:700; color:var(--text-dim); text-transform:uppercase;">${d.date}</span><div style="color:#fbbf24">${'★'.repeat(d.rating || 3)}${'☆'.repeat(5-(d.rating||3))}</div></div><h4 style="margin-bottom:8px; font-size:1.1rem;">${d.title}</h4><p style="color:var(--text-dim); line-height:1.5; font-size:0.95rem;">${d.summary}</p></div>`; });
        } catch (err) { logsHtml = '<div class="empty-state"><i data-lucide="lock" style="margin-bottom:10px;"></i><p style="color:var(--text-dim);">No permission to view activity logs.</p></div>'; }
        container.innerHTML = `<div class="profile-card-main" style="margin-top:0; margin-bottom:32px; display:flex; align-items:center; gap:32px;"><div class="profile-avatar-wrapper" style="margin-bottom:0; width:80px; height:80px; font-size:2rem;">${(s.name || 'S')[0]}</div><div><h2 style="font-size:2rem; font-weight:800; margin-bottom:4px;">${s.name}'s Activity</h2><p style="color:var(--text-dim); font-size:1.1rem;">Professional growth timeline and achievement records</p></div></div><div class="pulse-timeline">${logsHtml || '<div class="empty-state"><i data-lucide="activity"></i><p>No activity logs found.</p></div>'}</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();
    },
    ...
    showPulseEntryForm(staffId) {
        AppDialog.confirm({
            ...
            content: `<div class="form-group" style="margin-bottom:15px;"><label>Entry Title</label><input type="text" id="p-title" class="form-control" placeholder="Achievement or Milestone..."></div><div class="form-group" style="margin-bottom:15px;"><label>Summary / Notes</label><textarea id="p-summary" class="form-control" rows="4"></textarea></div><div class="form-grid-2"><div class="form-group"><label>Rating (1-5)</label><input type="number" id="p-rating" class="form-control" value="3" min="1" max="5"></div><div class="form-group"><label>Entry Date</label><input type="date" id="p-date" class="form-control" value="${new Date().toISOString().split('T')[0]}"></div></div>`,
            onConfirm: () => {
    ...
    async renderReport(id) {
        ...
        const userData = window.currentUserData || {}, isAdmin = userData.isAdmin, feePerms = userData.permissions?.fees_accounting || {}, canViewFees = isAdmin || feePerms === true || feePerms.view, staffPerms = userData.permissions?.staff_directory || {}, canViewPulse = isAdmin || staffPerms.pulse;
        let sFund = 0, sSpend = 0;
        if (canViewFees) { sFund = window.feesManager?.expenses?.filter(e => e.staffId === id && e.type === 'funding').reduce((a,b)=>a+b.amount, 0) || 0; sSpend = window.feesManager?.expenses?.filter(e => e.staffId === id && e.type === 'spend').reduce((a,b)=>a+b.amount, 0) || 0; }
